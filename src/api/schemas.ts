...
export const BrainEntrySchema = z.object({
  ...
  remaining_runs: z.number().int().nonnegative().nullable().optional().openapi({ description: "Derived remaining attempts before max_runs is exhausted (null when unlimited)", example: 0 }),
  completed_reason: z.string().optional().openapi({ description: "Derived terminal reason when a cron can no longer execute", example: "max_runs limit reached (1)" }),
  window_starts_at_utc: z.string().optional().openapi({ description: "Normalized UTC start window for display", example: "2026-02-23T00:00:00.000Z" }),
  window_expires_at_utc: z.string().optional().openapi({ description: "Normalized UTC end window for display", example: "2026-02-28T23:59:59.000Z" }),
...
export const CreateEntryRequestSchema = z.object({
  ...
  complete_on_idle: z.boolean().optional().openapi({ description: "Mark task completed instead of blocked when agent goes idle", default: false }),
  user_original_request: z.string().optional().openapi({
...
export const UpdateEntryRequestSchema = z.object({
  ...
  complete_on_idle: z.boolean().optional().openapi({ description: "Mark task completed instead of blocked when agent goes idle" }),
...
export const TaskTriggerResponseSchema = z.object({
  taskId: TaskIdSchema,
  run: CronRunSchema,
  pipeline: z.array(TaskSchema),
  pipelineCount: z.number(),
  message: z.string(),
}).openapi("TaskTriggerResponse");
