...
describe('App - Schedule View Mode (C key)', () => {
  it('toggles from task view to schedule view', async () => {
    const { stdin, lastFrame, unmount } = render(<App config={defaultConfig} />);
    ...
    const frame = lastFrame() || '';
    expect(frame).toContain('No scheduled tasks found');
  ...
  it('supports j/k navigation in schedule view without crashing', async () => {
    const { stdin, lastFrame, unmount } = render(<App config={defaultConfig} />);
  ...
  it('toggles back to task view with second C press', async () => {
    const { stdin, lastFrame, unmount } = render(<App config={defaultConfig} />);

    stdin.write('C');
    await new Promise(r => setTimeout(r, 10));
    expect(lastFrame() || '').toContain('schedules');

    stdin.write('C');
    await new Promise(r => setTimeout(r, 10));
    expect(lastFrame() || '').not.toContain('schedules');
