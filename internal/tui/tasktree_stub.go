...
func (tt *TaskTree) renderGroupedTaskLineWithProject(task types.ResolvedTask, isSelected bool, selectedTasks map[string]bool, showCheckboxes bool, activeProjectID string, width int) string {
	...
	if !tt.TextWrap && width > 0 {
		// Overhead: selMarker(2) + checkbox + indicator(2) + space(1) + badges + projectLabel + suffix
		overhead := 2 + len(checkboxPart) + 2 + 1 + len(cronPlain) + len(oncePlain) + len(windowPlain) + len(tzPlain) + len(projectLabelPlain)
	...
	return fmt.Sprintf("%s%s%s %s%s%s%s%s%s%s", selMarker, checkboxPart, indicatorStyled, cronStyled, onceStyled, windowStyled, tzStyled, projectLabel, title, prioritySuffix)
}
