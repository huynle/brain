...
describe("TaskRunner - schedule polling infrastructure", () => {
  ...
  test("poll invokes checkScheduledTasks during healthy poll", async () => {
    globalThis.fetch = createMockFetch({
      ...
      "/next": () =>
        Promise.resolve(
          new Response(JSON.stringify({ task: null, message: "No ready tasks" }), {
            status: 404,
          })
        ),
    });
...
describe("TaskRunner - scheduled task trigger and overlap", () => {
  ...
  test("checkScheduledTasks advances next_run after trigger", async () => {
    ...
    const nextRunUpdates = patchCalls.filter(c => {
      const b = c.body as Record<string, unknown>;
      return b?.next_run && typeof b.next_run === "string";
    });
    expect(nextRunUpdates.length).toBeGreaterThanOrEqual(1);

    // New next_run should be after the original (2025-01-15T09:00:00Z)
    const newNextRun = new Date((nextRunUpdates[0].body as { next_run: string }).next_run);
...
describe("TaskRunner - run finalization metadata fallback", () => {
  ...
  test("checkTuiTasks treats dead pid task as completed when run_finalizations marks current run completed", async () => {
    const runner = new TaskRunner({ projectId: "test-project", config, mode: "tui" });

    const task: RunningTask = {
      id: "task-finalized-dead-pid",
      path: "projects/test-project/task/task-finalized-dead-pid.md",
      title: "Finalized Dead PID Task",
      priority: "medium",
      projectId: "test-project",
      pid: 99999,
      startedAt: new Date().toISOString(),
          ...
          new Response(
            JSON.stringify({
              ...
              run_finalizations: {
                run_20260225_003: {
                  status: "completed",
                  finalized_at: "2026-02-25T10:20:00.000Z",
